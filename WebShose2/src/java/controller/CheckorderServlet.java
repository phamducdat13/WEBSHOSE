...
public class CheckorderServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Kiểm tra xem đối tượng Account có được lưu trong session không
        if (a == null) {
            // Nếu không, điều hướng người dùng đến trang đăng nhập
            response.sendRedirect("Login.jsp");
            return;
        }
        int id = a.getAccountid();
        ProductDAO productData = new ProductDAO();
        List<Cart> list = productData.getOrderByAccountID(id);
        // Đặt danh sách đơn hàng vào thuộc tính request để sử dụng trong JSP
        request.setAttribute("orderList", list);
        // Điều hướng người dùng đến trang Checkorder.jsp
